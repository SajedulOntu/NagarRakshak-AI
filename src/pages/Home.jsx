...
const features = [
  {
    ...
    desc: "Computer vision models scan drone footage in real time to flag road damage, waterlogging, and streetlight faults automatically.",
    color: "#22d3ee",
  },
  {
    icon: Building2,
    title: "Multi-Authority Access",
    desc: "Super Admin, DNCC North, DNCC South, and Maintenance teams each get a dedicated dashboard tuned to their zone and responsibilities.",
    color: "#ffb020",
  },
  {
    icon: BellRing,
    title: "Instant Alerts",
    desc: "The moment an issue is detected, the right city corporation is notified with location, severity, and imagery attached.",
    color: "#ff3b5c",
  },
  {
    icon: Radar,
...
const stats = [
  ...
  { label: "City Corporations Connected", value: "2" },
  ...
  return (
    <div className="min-h-screen bg-[#020617] hud-grid-bg text-white overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-24 pb-20 relative">
        <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.25, 0.5, 0.25] }}
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}
          ...
          <motion.div animate={{ y: [0, -14, 0] }} transition={{ duration: 3, repeat: Infinity }} className="flex justify-center mb-8">
          ...
          <p className="mt-5 text-gray-400 max-w-2xl mx-auto">
            Detect road hazards, pinpoint locations, and automatically notify DNCC North & South authorities — before small problems become city-wide ones.
          </p>

          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} onClick={() => navigate("/login")}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              <div className="text-2xl sm:text-3xl font-display font-bold text-cyan-300">{stat.value}</div>
              <div className="text-gray-400 text-xs mt-1 font-hud tracking-wide uppercase">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
          ...
          {features.map((f, i) => {
            ...
            return (
              <motion.div key={f.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="absolute -top-8 -right-8 w-28 h-28 rounded-full blur-2xl opacity-25 group-hover:opacity-45 transition-opacity"
                  style={{ background: f.color }} />
                <div className="p-3 rounded-xl inline-flex relative z-10" style={{ background: `${f.color}22` }}>
